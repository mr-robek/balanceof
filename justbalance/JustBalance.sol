...
contract JustBalance is Ownable {
  ...
  function balanceOf(address token, address user) public constant returns (uint256, uint256) {
    uint256 walletBalance = token == 0x0 ? userBalance(user) : Token(token).balanceOf(user);
    uint256 edBalance = etherdelta.balanceOf(token, user);
    return (walletBalance, edBalance);
  }

  function userBalance(address user) public constant returns (uint256) {
      return user.balance;
  }
}
